...
public class TamilTokenizer {
    ...
    private boolean isATamilLetter(String s) {
        if(s.length() != 1) return true;
        int tamilLowerBound = 2994;
        int tamilaUpperBound = 3071;

        int cp = s.codePointAt(0);
        if(cp >= tamilLowerBound && cp <= tamilaUpperBound) {
            return true;
        }
        return false;
    }
    ...
    public LinkedList<String> splitSentences(String str) {
        ...
        for(String sentence : parts) {
            ...
            if(containsTamil(sentence)) {    // filter empty sentences and non-sinhala sentences
